unit module ONNX::Runtime::API::Simple;

use NativeCall;
use ONNX::Runtime::Types;

# Use hardcoded path for now - the dynamic lookup was causing issues
...
# Simple API wrapper with just the functions we need
class SimpleAPI {
    has $.api;
    has %!funcs;
    ...
    has %!indices = (
        ...
        'Run' => 9,
        # We'll skip release functions for now and rely on GC
    );
    ...
    method get-func($name) {
        note "get-func: Looking for $name" if %*ENV<ONNX_DEBUG>;
        return %!funcs{$name} if %!funcs{$name}:exists;
        
        my $idx = %!indices{$name} or die "Unknown function: $name";
        note "get-func: Index for $name is $idx" if %*ENV<ONNX_DEBUG>;
        my $offset = $idx * nativesizeof(Pointer);
        my $ptr = Pointer.new($!api.Int + $offset);
        note "get-func: Reading function pointer from offset $offset" if %*ENV<ONNX_DEBUG>;
        my $func = nativecast(CArray[Pointer], $ptr)[0];
        note "get-func: Got function pointer: $func" if %*ENV<ONNX_DEBUG>;
    ...
    method create-env(int32 $log-level, Str $logid, Pointer[OrtEnv] $env-ptr is rw) {
        note "create-env: Getting function..." if %*ENV<ONNX_DEBUG>;
        my $func-ptr = self.get-func('CreateEnv');
        note "create-env: Got function pointer: ", $func-ptr if %*ENV<ONNX_DEBUG>;
        my &func = nativecast(
            :(int32, Str, Pointer[OrtEnv] is rw --> OrtStatus),
            $func-ptr
        );
        note "create-env: Calling with log-level=$log-level, logid=$logid" if %*ENV<ONNX_DEBUG>;
