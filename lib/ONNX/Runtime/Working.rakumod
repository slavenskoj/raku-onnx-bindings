unit module ONNX::Runtime::Working;
...
# Module-level state (initialized on first use, not during compilation)
my $API;
my %FUNCS;
...
sub get-func(Str $name) {
    ...
    return %FUNCS{$name} // die "Unknown function: $name";
}

# Runtime Session class
class Session is export {
    ...
    method new(Str :$model-path!, :$log-level = WARNING) {
        ensure-initialized();
