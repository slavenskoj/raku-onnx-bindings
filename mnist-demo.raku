...
# ONNX Runtime C Bindings for Raku
# This demonstrates loading and running the MNIST model
...
say "Model loaded!";
say "  Input: $input-name (28x28 image)";
say "  Output: $output-name (10 class probabilities)";
...
# Create input tensor - a simple pattern that should be recognized as a digit
say "\nCreating test input (28x28 image)...";
my @image;
for ^28 -> $y {
    my @row;
    for ^28 -> $x {
        # Create a simple pattern - a vertical line in the middle (like digit 1)
        if 10 <= $x <= 18 && 5 <= $y <= 23 {
            @row.push(1.0);
        } else {
            @row.push(0.0);
...
# Display results
say "\n=== Results ===";
say "Prediction probabilities:";
my @probs;
for ^10 -> $i {
    @probs.push($out-array[$i]);
    say "  Digit $i: {$out-array[$i].fmt('%6.4f')}";
}

my $max-idx = (^10).max({ @probs[$_] });
say "\nPredicted digit: $max-idx";
say "Confidence: {@probs[$max-idx].fmt('%.2%')}";
